      ...
      options = {
        ...
        owner = lib.mkOption {
          ...
          apply =
            ...
            if builtins.isString val then
              config.users.users.${val}.id or (
                if intVal.success then
                  intVal.value
                else
                  throw ''
                    agefs requires a UID to be set, but user ${val} does not have one.
                  ''
              )
            else
              val;
        ...
        group = lib.mkOption {
          ...
          apply =
            ...
            in
            if builtins.isString val then
              config.users.groups.${val}.id or (
                if intVal.success then
                  intVal.value
                else
                  throw ''
                    agefs requires a GID to be set, but group ${val} does not have one.
                  ''
              )
            else
              val;
  ...
  config = lib.mkIf (cfg.secrets != { }) {
    launchd.daemons.agefs = {
      ...
      serviceConfig = {
        ...
        StandardOutPath = "/tmp/agefs.log";
        StandardErrorPath = "/tmp/agefs.log";
      };
    };
  };
}
